...
// SkyGuard backend origin the dev server proxies /api and /ws to. Override with
// the BACKEND_ORIGIN env var (e.g. BACKEND_ORIGIN=http://192.168.1.20:8000).
// The browser talks to Vite same-origin, so no CORS config is needed in dev.
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        // WebSocket telemetry stream -> backend (ws: true enables upgrade proxying)
        '/ws': {
          target: BACKEND_ORIGIN,
          changeOrigin: true,
          ws: true,
